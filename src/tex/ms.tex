...
To illustrate this, we generate a catalog of \result{1,000} GW events from an underlying mass distribution described by the \plp{} model, with hyperparameters consistent with those found in \citet{o3b_pop}.
This simulated catalog is designed to be representative of the gravitational wave transient catalog after the \ac{LVK}'s fifth observing run.
...
With \result{1,000} events, we find that using a parametric model of the same form as the true simulated distribution (\plp) recovers the injected value of \Ho{} within \result{X-$\sigma$}.
...
We fit the non-parametric population model described in Section~\ref{sec:model} to a simulated GW catalog of \result{1,000} GW events from an underlying mass distribution described by the \plp{} model, with hyperparameters consistent with those found in \citet{o3b_pop}.
This simulated catalog is designed to be representative of the gravitational wave transient catalog after the \ac{LVK}'s fifth observing run.
The details of the data simulation are described in Appendix~\ref{ap:data generation}.
...
This result also implies that we can expect to measure \Ho{} to \result{X\%} by the end of the \ac{LVK}'s fifth observing run using this method.
...
\begin{itemize}
    ...
    \item We have also predicted a \result{X\%} measurement of \Ho{} by the end of the \ac{LVK}'s fifth observing run using non-parametric spectral sirens.
