...
%When applied to data from next-generation detectors, this method will measure cosmological parameters that govern $H(z)$.
...
We find that the \bpl{} model produces an over- or under-estimate of \Ho{} at greater than $1\sigma$ $\variable{output/BPL_bias_percent.txt}\%$ of the time, %whereas the \ac{GP}-based model reaches that level of bias only \result{Z}\% of the time, 
...
\begin{figure*}
    ...
    The posterior on \Ho{} is relatively unchanged between the two, and only minor differences are observed in the mass spectra.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/bias.pdf}
