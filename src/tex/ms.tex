...
% \daniel{No need to know: astrophysically-agnostic gravitational wave cosmology\\
% No need to know: gravitational-wave cosmology without astrophysics\\
% No need to know: gravitational-wave cosmology without astrophysical assumptions}}
% Ignorance is bliss: astrophysics-agnostic gravitational wave cosmology
...
This can be done in the absence of electromagnetic measurements: features in the mass distribution of \ac{GW} sources can provide self-calibrating redshift measurements without the need for an accompanying electromagnetic transient or catalog of potential host galaxies.
...
% Furthermore, i
It is commonly presumed that the exact shape of the mass distribution must be known \emph{a priori}\/ to obtain unbiased measurements of cosmological parameters in this fashion.
We demonstrate that spectral sirens can accurately infer cosmological parameters without assuming
prior knowledge of the \ac{GW} mass distribution.
By adopting a flexible, Gaussian process-based model for the mass distribution of compact binaries, we find that we can accurately and agnostically infer the mass distribution while simultaneously measuring parameters governing cosmological expansion.
We demonstrate this analysis using a simulated catalog of 1,000 gravitational-wave events consistent with expectations for the next \acl{LVK} observing run.
We find that both the source mass model and cosmological parameters are correctly reconstructed, and predict a $\variable{output/nonparh0percent.txt}\%$ measurement of \Ho{}, keeping all other cosmological parameters fixed, and a $\variable{output/Hz_percent.txt}\%$  measurement of $H(z=\variable{output/mostsensitivez.txt})$ when fitting for multiple cosmological parameters ($1\sigma$ uncertainties).
This astrophysically-agnostic spectral siren technique will be essential to arrive at precise and unbiased cosmological constraints from gravitational-wave source populations.
\end{abstract}
...
We find that using our non-parametric model allows for a $\variable{output/nonparh0percent.txt}\%$ measurement of \Ho{} and a $\variable{output/Hz_percent.txt}\%$ measurement of $H(z=\variable{output/mostsensitivez.txt})$ during \ac{O5}, when the detectors will reach their design sensitivity. %, and a \result{X}\% measurement with only \result{X} weeks of observing with a next-generation detector network. 
...
The code used to generate all simulated data, perform the population inference, and create all figures in this paper is made publicly available at \GitHubURL{githubLink}, and was enabled by the \showyourwork package \citep{Luger2021}.
...
% \daniel{We should say more about the importance of this assumption?}
...
\comment{[These numbers are placeholders for now and will be updated in the next version.]}
