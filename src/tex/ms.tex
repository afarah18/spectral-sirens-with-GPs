...
\new{Additionally, Appendix~\ref{ap:mass ratio} demonstrates that including secondary masses in the fit does not change the results of the analysis presented here.}
...
of the time.
\new{Additionally, the \bpl{} model typically overestimates \Ho{}, while the \plp{} model produces a roughly equal number of over- and underestimates of \Ho{}, implying a systematic bias in the incorrect but not the correct parametric model.}\footnote{\new{
...
For simplicity, we additionally assume that \new{the population of mass ratios is peaked at $q=0.9$, which is consistent with current observations.}
Thus, $\theta = \{m_1,z\}$.
...
\label{ap:mass ratio}
We examine the effect of only fitting the distribution of primary masses on our results.
To do so, we perform two spectral siren analyses: one that includes a fit to secondary mass, and one that does not.
Both analyses use the parametric, \plp{} model for the distribution of primary masses.
We model the distribution of secondary masses with a power law in mass ratio, similarly to the majority of analyses presented in \citet{abbott_population_2021,abbott_population_2023}. 

We use the same set of simulated events for both analyses.
These are generated in the same way as described in Section~\ref{sec:data generation}, but with a uniform mass ratio distribution (rather than assuming all mass ratios are unity) in order to understand the impact of our assumptions on the results over the full range of possible mass ratios.
We correspondingly modify our injection set to cover the same mass ratio range.

The results are shown in Figure~\ref{fig:mass ratio}.
...
We therefore conclude that fitting for secondary masses does not significantly impact our main conclusions, as is to be expected from a self-consistent hierarchical analysis when the data do not exhibit correlations.
