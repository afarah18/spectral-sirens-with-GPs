...
Once paired with their redshifts, a Hubble diagram can be constructed to directly probe the Universe's expansion.
This can be done in the absence of any electromagnetic information: features in the mass distribution of \ac{GW} can provide self-calibrating redshift measurements without the need for an accompanying electromagnetic transient or catalog of potential host galaxies.
\sout{So far, this technique has only been applied with simple parametric representations of the mass distribution. 
...
It is commonly presumed, however, that the exact shape of the mass distribution must be known \emph{a priori} to obtain unbiased measurements of cosmological parameters in this fashion.
...
By adopting a flexible, Gaussian-process model for the mass distribution of compact binaries, we find that we can accurately and agnostically learn this mass distribution while simultaneously measuring parameters governing cosmological expansion.
We demonstrate this analyses using a simulated catalog of $10^3$ gravitational-wave events consistent with expectations for the next \acl{LVK} observing run.
...
Cosmological measurements with gravitational waves traditionally require independent electromagnetic measurement of source redshifts.
A well-known demonstration was the multi-messenger event GW170817 \citep{abbott_multi-messenger_2017,coulter_swope_2017,valenti_discovery_2017,2017ApJ...848L..27T}, whose clear association with a host galaxy provided a precise redshift measurement and allowed for a direct measurement of the Hubble constant, \Ho{} \citep{abbott_gravitational-wave_2017}.
External redshift information can also come from galaxy catalogs, which provide redshift distributions for each \ac{GW} signal and allow for a probabilistic measurement of \Ho{} when multiple \ac{GW} detections are combined \citep[e.g.][]{del_pozzo_inference_2012, chen_two_2018, fishbach_standard_2019, gray_cosmological_2020, gwtc3_cosmo, gair_hitchhikers_2023}.

However, external electromagnetic information about \ac{GW} source redshifts need not be available in order to use them as standard sirens.
...
In practice, the source-frame masses of individual events are not known~\citep[unless tidal information is available][]{messenger_measuring_2012}.
...
Indeed, first principles models for the mass distribution of merging compact binaries are not available: significant theoretical uncertainties exist on the shapes, locations, and very existences of predicted features like a ``pile-up'' due to pulsational-pair instability, a pair-instability-driven upper mass gap, or the maximum neutron star mass.
To this end, spectral siren cosmology\footnote{And also statistical cosmological measurements leveraging galaxy catalogs.} relies on \textit{simultaneously} measuring a source-frame mass spectrum alongside cosmological parameters.
...
We have repeated this analysis with 50 separate simulated catalogs of $\sim500$ events, and found that the broken power law model produces an overestimate of \Ho{} at greater than $1\sigma$ $\variable{output/BPL_bias_percent.txt}\%$ of the time of the time, %whereas the \ac{GP}-based model reaches that level of bias only \result{Z}\% of the time, 
...
We demonstrate this by repeating the same \ac{GP}-based spectral siren analysis of the previous subsection while also fitting for the local matter density, \Omm, but we emphasize that it would also have been possible to constrain other cosmological parameters governing $H(z)$.
...
\comment{Projected DESI errors are $\sim3\%$ at $z=1$, which are comparable with our estimated error of $\variable{output/Hz_percent.txt}\%$, but those won't be realized until 202X.}
...
We do not fit for spins, as they do not carry additional cosmological information, and additionally neglect secondary masses.
