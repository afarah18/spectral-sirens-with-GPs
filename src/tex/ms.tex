...
\jme{[Add some numbers about expected precision (e.g. few percent with O5, sub-percent 3G). Emphasize spectral sirens allow you to measure expansion rate at high redshift - inaccesible by other means!]}
...
\jme{[I agree. I'd follow: 1) Intro 2) Spectral sirens (intro to the methods) 3) Non-parametric spectral sirens (new methods part) 4) Results (O5 forecast comparing non-parametric, right parametric, wrong parametric) 5) Conclusions and future prospects (discuss going to non-parametric 2D) + Appendices (any technical part regarding details on the analysis)]}
...
\label{sec:model}
\jme{[Suggestion to bump up this to its own section to gain visibility - see proposed structure in previous comment]}
...
\jme{[I think it would be relevant to compare the forecast of the non-parametric and the right parametric model. Do we loose constrianing power by being flexible (probably)? how much?]}
